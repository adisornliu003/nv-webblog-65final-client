<template>
    <div style="border:20px solid #99CCFF">
      ...
      <form v-on:submit.prevent="editSpeaker">
        <p>ชื่อรุ่น<input type="text" v-model="speaker.title" /></p>
  
        <transition name="fade">
          <div class="thumbnail-pic" v-if="speaker.thumbnail != 'null'">
            <img :src="BASE_URL + speaker.thumbnail" alt="thumbnail" />
          </div>
        </transition>
        ...
        <p>ที่อยู่: <input type="text" v-model="speaker.category" /></p>
        <p>ประเภทร้าน: <input type="text" v-model="speaker.status" /></p>
        <p>
          <button class="button1" type="submit">ยืนยัน</button>
          <button class="button1" v-on:click="navigateTo('/speakers')">กลับ</button>
        </p>
      </form>
    </div>
  </template>
  ...
  export default {
    data() {
      return {
        BASE_URL: "http://10.31.27.240:8081/assets/uploads/",
        error: null,
        // uploadedFiles: [],
        uploadError: null,
        currentStatus: null,
